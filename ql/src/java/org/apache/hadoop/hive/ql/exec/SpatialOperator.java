 ...
 *
 */
public class SpatialOperator extends Operator<SpatialDesc> implements
    Serializable {
  ...
  // Input to the script
  transient Serializer [] scriptInputSerializer=null;
  ...
  protected void initializeOp(Configuration hconf) throws HiveException {
    ...
    if (scriptInputSerializer ==null)
     {
      scriptInputSerializer = new Serializer [2]; // init the array
    }
    ...
    try {
      ...
      scriptInputSerializer[0] = (Serializer) ((conf.getScriptInputInfos()[0])
          .getDeserializerClass().newInstance());
      scriptInputSerializer[0].initialize(hconf, (conf.getScriptInputInfos()[0])
          .getProperties());

      scriptInputSerializer[1] = (Serializer) ((conf.getScriptInputInfos()[1])
          .getDeserializerClass().newInstance());
      scriptInputSerializer[1].initialize(hconf, (conf.getScriptInputInfos()[1])
          .getProperties());
  ...
  public void processOp(Object row, int tag) throws HiveException {
    ...
    try {
      Writable res = scriptInputSerializer[tag].serialize(row,
          inputObjInspectors[tag]);
      // ((TaggedRecordWriter)scriptOutWriter).write(res,tag);
