...
hadoop dfs -rmr ${hdfsoutdir}
 ...
 #hadoop jar ${hadooppath}/contrib/streaming/hadoop-streaming-*.jar -mapper 'cat - ' -file /bin/cat -reducer 'map_contains '${query_polygon}' '${index}'' -file /Users/hixiaoxi/Documents/GitHub/hivesp/resque/xiling/task3/map_contains -input ${input} -output ${hdfsoutdir} -verbose -cmdenv LD_LIBRARY_PATH=/usr/local/lib:$LD_LIBRARY_PATH -jobconf mapred.job.name="map_contains"
